...
function GetCFGFromNode(Trys:integer=5):string;
...
var
  ...
  DefaultNodes  : String =  '47.87.181.190;8080:47.87.178.205;8080:66.151.117.247;8080:47.87.180.219;8080:47.87.137.96;8080:192.3.85.196;8080:192.3.254.186;8080:198.46.218.125;8080:140.99.161.224;8080:63.227.69.162;8080:20.199.50.27;8080:45.83.151.30;8080:107.172.21.121;8080:';
  ActiveNodesStr  : string = '';
  DefaultCclasses : string = '';
  CCBlocked : integer = 0;
  VPNsThreadRunning : boolean = false;
...
var
  ...
  NewNode : TNodeData;
Begin
ActiveNodesStr := linextext;
linextext := StringReplace(linextext,':',' ',[rfReplaceAll, rfIgnoreCase]);
linextext := Trim(linextext);
EnterCriticalSection(CS_NodesArray);
...
REPEAT
   ...
   EXCEPT on E:Exception do
      begin
      WasDone := False;
      //ToLog('.Error GetMNsFromNode: '+E.Message);
      end;
   END{Try};
Inc(Trys);
UNTIL ( (WasDone) or (Trys = 5) );
